...
class Controller:
    ...
    def handleCompConnessa(self,e):
        ...
        try:
            ...
            self._view.txt_result.controls.clear()
            self._view.txt_result.xontrols.append(ft.Text("Il valore inserito non è un numero", color = "red"))
            self._view.update_page()
            return

        if not self._model.hasNode(idInput):
            self._view.txt_result.controls.clear()
            self._view.txt_result.xontrols.append(ft.Text("L'id inserito non corrisponde a un nodo del grafo", color="red"))
            self._view.update_page()
            return

        sizeCompConnessa = self._model.getInfoConnessa(idInput)
        self._view.txt_result.controls.clear()
        self._view.txt_result.controls.append(ft.Text(f'La componente connessa che contiene il nodo {self._model.getObjectFromId(idInput)} ha dimensione connessa pari a {sizeCompConnessa}'))
        self._view.update_page()
